...
from new_loss import L_spa, L_exp
...
# Model architecture settings (proposed model: --dynamic_filter --both_sty_con --style_attention)
...
parser.add_argument('--style_attention', action='store_true', help='use both style and content attention filter in the decoder')
...
parser.add_argument('--spa_loss', action='store_true', help='use the spatial consistency loss')
parser.add_argument('--exp_loss', action='store_true', help='use the Exposure Control loss')
'''Zero-DCE spatial consistency loss
   Zero-Reference Deep Curve Estimation for Low-Light Image Enhancement'''

parser.add_argument('--ssim_loss', action='store_true', help='use the self ssim')
parser.add_argument('--recon_ssim_loss', action='store_true', help='use the recon ssim')
...
parser.add_argument('--tvWeight', type=float, default=20, help='tv loss weight')
...
# Build model
style_net = TransformerNet(
                           style_attention=opt.style_attention, 
                           train_only_decoder=opt.train_only_decoder,
                           style_content_loss=opt.style_content_loss, 
                           spa_loss=opt.spa_loss,
                           exp_loss=opt.exp_loss,
                           ssim_loss = opt.ssim_loss,
                           recon_loss=opt.recon_loss,
                           recon_ssim_loss=opt.recon_ssim_loss, 
...
def LoadPretrained(checkpoint):
    try:
        style_net.load_state_dict(torch.load(checkpoint[0], map_location=lambda storage, loc: storage), strict=False)
    except:
        try:
            tmp_style_net = TransformerNet(
                               style_attention=False, 
...
for epoch in range(opt.load_epoch+1, opt.epoches+1):
    ...
    for iteration, sequence in enumerate(loader):
        ...
        StyledFirstFrame, RelaxedStyledFirstFrame, ReconFirstFrame, ReconFirstStyle, \
            content_loss, new_style_loss, recon_loss, recon_ssim_loss, old_style_loss = style_net(FirstFrame, Style)
        ...
        if opt.temporal_loss:
            SecondFrame_ori, SecondFrame, ForwardFlow = TemporalLoss.GenerateFakeData(FirstFrame)
            ...
            # loss_spa = torch.mean(L_spa(StyledSecondFrame, SecondFrame))
            if opt.ssim_loss:
                res = SecondFrame_ori - FirstFrame
                res = RGB2Gray(res)
                res_s = StyledSecondFrame - StyledFirstFrame
                res_s = RGB2Gray(res_s)
                RES = res - res_s
                ssim_loss = 1-SSIM(res, res_s) + TV(RES)
                

            Loss = Loss + temporal_loss * opt.temporalWeight + ssim_loss * 60
        else:
            temporal_loss = 0.
            temporal_loss_GT = 0.
        ...
        if opt.spa_loss:
            ...
            Loss = Loss + spa_loss * 60 
        
        if opt.exp_loss:
            exp_loss = torch.mean(L_exp(StyledFirstFrame))
            Loss = Loss + exp_loss*20
        ...
        if opt.recon_ssim_loss:
            Loss = Loss + recon_ssim_loss * 20
        ...
        if iteration % 10 == 0:
            ...
            print("[Epoch %d/%d][Iter %d/%d] New Style: %.3f, Content: %.3f, Old Style: %.3f, Recon: %.3f, ReconSSIM: %.3f, SSIM: %.3f, TV: %.3f, Temporal: %.3f (%.3f), GAN: %.3f" \
        ...
        if iteration % opt.log == 0 and iteration != 0:
