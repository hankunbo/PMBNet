...
class Decoder(nn.Module):
    ...
    def compute(self, x, style_features_map=None, style_features=None):
        # x = self.AdaIN_compute(x, style_features.relu4_1, 1)        # h = self.AdaIN_filter_compute(x, style_features.relu4_1, style_features.map)
        h1 = self.MSANet4_1(x, style_features_map)
        h2 = self.SANet4_1(x, style_features_map)
        h3 = h1 + h2
        h4 = self.AdaIN_compute(h3, style_features.relu4_1, 1)
        h5 = self.slice5(h4)
        h = x + h5
    ...
    def forward(self, x, style_features=None):
        ...
        h3 = h1 + h2
        h4 = self.AdaIN(h3, style_features.relu4_1, 1)
        h5 = self.slice5(h4)
        h = x + h5
...
class TransformerNet(nn.Module):
    ...
    def add(self, patch, style):
        F_patch = self.Encoder(self.RGB2Gray(patch))
        F_spatch = self.EncoderStyle(style).map

        ## Store data as list in self.F_patches
        self.F_patches.append(F_patch)
        self.F_spatches.append(F_spatch)

    def compute(self):
        self.Decoder.compute(torch.cat(self.F_patches, dim=0), torch.cat(self.F_spatches, dim=0), self.F_style)
