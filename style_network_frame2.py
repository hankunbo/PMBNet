...
class Enhance_Module(nn.Module):                       ##加入通道注意力机制
    def __init__(self, dim_in, dim_out, rate=1, bn_mom=0.1):
        ...
        # self.branch5_bn = nn.InstanceNorm2d(dim_out, momentum=bn_mom)
    ...
    def forward(self, content_feat):
        ...
        # global_feature_s = self.branch5_bn(global_feature_s)
        ...
        # feature_cat_0 = torch.cat([conv1x1, conv1x1_s], dim=1)
        # feature_cat_1 = torch.cat([conv3x3_1, conv3x3_1_s], dim=1)
        # feature_cat_2 = torch.cat([conv3x3_2, conv3x3_2_s], dim=1)
        # feature_cat_3 = torch.cat([conv3x3_3, conv3x3_3_s], dim=1)
        # feature_cat_global = torch.cat([global_feature, global_feature_s], dim=1)
        #
        # cbam_0 = self.cbam(feature_cat_0)
        # cbam_1 = self.cbam(feature_cat_1)
        # cbam_2 = self.cbam(feature_cat_2)
        # cbam_3 = self.cbam(feature_cat_3)
        # cbam_global = self.cbam(feature_cat_global)

        feature_cat = torch.cat([conv1x1, conv3x3_1, conv3x3_2, conv3x3_3, global_feature], dim=1)
        # print('conv1:', conv1x1.shape)
        # print('conv2:', conv3x3_1.shape)
        # print('conv3:', conv3x3_2.shape)
        # print('conv4:', conv3x3_3.shape)
...
class Decoder(nn.Module):
    ...
    def forward(self, x, style_features=None):
        # print(x.shape)
        # print("style:",style_features.map.shape)

        x = self.Enhance_Module(x)
        sf = self.Enhance_Module(style_features.map)
        # print("stylef:", sf.shape)
        h1 = self.MSANet4_1(x, sf)
        h2 = self.SANet4_1(x, sf)
        h3 = h1 + h2
