...
checkpoint_path = "./Model/style_net-TIP-final-2.pth"
...
class VideoStylization:
	def __init__(self, checkpoint="./", style_img_pair=[], test_sequence="", img_type="png"):
		...
		self.transformer = Stylization(checkpoint=checkpoint_path, cuda=cuda, style_num=len(style_img_pair))
...
content_video = 'cal_temporal/sea'	
frame_list = glob.glob(content_video)

style_img_pair_list = [['cal_temporal/other_13.jpg', 'cal_temporal/other_11.jpg']]
test_sequence_list = frame_list

for style_id, style_img_pair in enumerate(style_img_pair_list):
	print(len(style_img_pair))
