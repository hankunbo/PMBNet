...
class SANet(nn.Module):
    ...
    def compute(self, content_feat, style_feat):
        F = self.f(mean_variance_norm(content_feat))
        G = self.g(mean_variance_norm(style_feat))
        H = self.h(style_feat)
        b, c, h, w = F.size()
...
class Enhance_Module(nn.Module):                       ##加入通道注意力机制
    ...
    def compute(self, content_feat, style_feat):
        ...
        feature_cat_0 = self.AdaIN_compute(conv1x1, self.cal_mean_std(conv1x1_s), 0)
        feature_cat_1 = self.AdaIN_compute(conv3x3_1, self.cal_mean_std(conv3x3_1_s), 1)
        feature_cat_2 = self.AdaIN_compute(conv3x3_2, self.cal_mean_std(conv3x3_2_s), 2)
        feature_cat_3 = self.AdaIN_compute(conv3x3_3, self.cal_mean_std(conv3x3_3_s), 3)
        feature_cat_global = self.AdaIN_compute(global_feature, self.cal_mean_std(global_feature_s), 4)

        del feature_cat_0, feature_cat_1, feature_cat_2, feature_cat_3, feature_cat_global
    ...
    def forward(self, content_feat, style_feat):
        print("content_feat.size()2", content_feat.size(), style_feat.size())  #这里没有任何问题
        [b, c, row, col] = content_feat.size()
        [b, c, row_s, col_s] = style_feat.size()
        conv1x1 = self.branch1(content_feat)
        conv3x3_1 = self.branch2(content_feat)
        ...
        feature_cat_0 = self.AdaIN(conv1x1, self.cal_mean_std(conv1x1_s), 0)
        feature_cat_1 = self.AdaIN(conv3x3_1, self.cal_mean_std(conv3x3_1_s), 1)
        feature_cat_2 = self.AdaIN(conv3x3_2, self.cal_mean_std(conv3x3_2_s), 2)
        feature_cat_3 = self.AdaIN(conv3x3_3, self.cal_mean_std(conv3x3_3_s), 3)
        feature_cat_global = self.AdaIN(global_feature, self.cal_mean_std(global_feature_s), 4)
...
class Decoder(nn.Module):
    def __init__(self, dynamic_filter=True ,out_channels = 3,
        *,
        dim = 512,
        content_feat_size = 32,
        heads = 4,
    ...
    def forward(self, x, style_features=None):
        if self.dynamic_filter:
            ...
            h = self.AdaIN(x, style_features.relu4_1, 1)
...
class TransformerNet(nn.Module):
    ...
    def add_Enhance_Module(self, patch):
        F_cpatch = self.Encoder(self.RGB2Gray(patch))

        self.F_cpatches.append(F_cpatch)

    def add_Decoder(self, patch):
        F_cpatch = self.Encoder(self.RGB2Gray(patch))
        F_cspatch = self.Enhance_Module(F_cpatch, self.F_style.map)

        self.F_cspatches.append(F_cspatch)
    
    def compute_Enhance_Module(self):
        Fc = torch.cat(self.F_cpatches, dim=0)
        
        self.Enhance_Module.compute(Fc, self.F_style.map)

    #This is after add
    def compute_Decoder(self):
        Fcs = torch.cat(self.F_cspatches, dim=0)
        
        self.DecoderA.compute(Fcs, self.F_style)
        self.DecoderM.compute(Fcs, self.F_style)
    ...
    def forward(self, input_frame):
        ...
        F_cs = self.Enhance_Module(F_content, self.F_style.map)

        styled_resultA = self.DecoderA(F_cs, self.F_style)
        styled_resultM = self.DecoderM(F_cs, self.F_style)
