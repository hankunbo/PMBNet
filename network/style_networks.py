...
class Decoder(nn.Module):
    ...
    def forward(self, x, style_features=None):
        ...
        h2 = self.AdaIN(x, style_features.relu4_1)
        h3 = self.slice5(h2)
        h4 = x + h3
...
class TransformerNet(nn.Module):
    def __init__(self, style_attention=True):
        ...
        self.DecoderA = Decoder(style_attention=style_attention, out_channels = 3)
        self.DecoderM = Decoder(style_attention=style_attention, out_channels = 6)
        self.Vgg19 = Vgg19()
    ...
    ## ---------------------------------------------------
    # freeze the parameters when training different Decoder
    ...
    def Train_DA(self):
        ...
        for param in self.Enhance_Module.parameters():
            param.requires_grad = True

        for param in self.EncoderStyle.parameters():
            param.requires_grad = True

        for param in self.DecoderM.parameters():
            param.requires_grad = False

        for param in self.DecoderA.parameters():
            param.requires_grad = True
    ...
    def validation(self, content, style):
        F_cur = self.Encoder(self.RGB2Gray(content))
        F_style = self.EncoderStyle(style)

        F_cs = self.Enhance_Module(F_cur, F_style.map)
        
        styled_resultA = self.DecoderA(F_cs, F_style)
        styled_resultM = self.DecoderM(F_cs, F_style)
        return styled_resultA, styled_resultM[:,:3,:,:], styled_resultM[:,3:,:,:]
    ...
    def forward(self, content, style):
        ...
        F_styledA = self.Vgg19(styled_resultA)
        F_styledM = self.Vgg19(styled_resultM[:,:3,:,:])

        F_content_gt = self.Vgg19(gray_content)

        F_style_gt = self.Vgg19(style)
        
        recon_contentA = self.DecoderA(F_content, self.EncoderStyle(content))
        recon_styleA = self.DecoderA(self.Encoder(self.RGB2Gray(style)), F_style)
        recon_contentM = self.DecoderM(F_content, self.EncoderStyle(content))
        recon_styleM = self.DecoderM(self.Encoder(self.RGB2Gray(style)), F_style)
        
        return styled_resultA, styled_resultM, \
                F_styledA, F_styledM, F_content_gt, F_style_gt,\
                recon_contentA, recon_styleA, recon_contentM, recon_styleM
